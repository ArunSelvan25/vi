/**
 * Attacks the Supabase backend (supabase/functions/api/backend.js) on a real
 * Postgres: setup takeover, brute force, enumeration, forged tokens, role
 * escalation, and reaching the tables around the backend. Ported from
 * security-test.mjs, which runs the same attacks on the Apps Script backend.
 */
...
await check('hashes are stretched, not a single round', async () => {
  ...
  assert(await b.hashPasswordV2('correct-horse-battery', salt) !== stored, 'stored as the Apps Script scheme');
});
...
await check('an account migrated from the sheet (Apps Script v2 hash) signs in, and is upgraded', async () => {
  // Every existing user arrives with the password hash Code.gs wrote. Nobody
  // may have to reset their password because the backend moved.
  const b = await makeSandbox();
  await b.handle('setup', { adminPhone: '9880011111', adminPassword: 'correct-horse-battery' }, '');
  const admin = (await b.handle('login', { phone: '9880011111', password: 'correct-horse-battery' }, '')).data.token;
  await b.handle('createUser', { name: 'Sheet', phone: '9000000009', role: 'manager',
                                 password: 'from-the-sheet-1234' }, admin);
  const u = (await b.readTable('Users')).find(x => x.phone === '9000000009');
  const sheetSalt = 'uuid-from-apps-script';
  const v2 = await b.hashPasswordV2('from-the-sheet-1234', sheetSalt);
  // what Code.gs's hashPassword produces for this password and salt, so the port cannot drift from it
  assert(v2 === 'v2$ECl2fNv8mgV9ZY7GAVjb06iBiAGSRaDWQm1tCOCKmSo=', 'differs from the Apps Script hash: ' + v2);
  await b.query('update app_users set salt = $1, password_hash = $2 where id = $3', [sheetSalt, v2, u.id]);
