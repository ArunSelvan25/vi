/**
 * Production-readiness probes for the Supabase backend: the failures that only
 * appear once real people with real roles use the app, rather than a single
 * administrator in testing.
 *
 * Ported from production-test.mjs, which runs the same probes against the
 * Apps Script backend. Probes about spreadsheet mechanics (tabs, formulas in
 * cells, block writes, the script lock) are replaced by their Postgres
 * equivalents or dropped where the problem cannot exist any more.
 */
import fs from 'fs';
...
await probe('setup on a database that already holds data changes none of it', async () => {
  const box = await makeSandbox();
  // data imported from the sheet before anyone has signed in
...
await probe('signing in can return the workbook in the same response', async () => {
...
await probe('a change made directly in the table editor is picked up', async () => {
  const { box, c } = await billedLease();
  const full = (await c('bootstrap', {})).data;
  await box.query(`update tenants set occupation = 'Typed in the sheet'`);
  const again = (await c('bootstrap', { known: full.hashes })).data;
...
console.log('\n— meter readings —');
await probe('a round of readings bills occupied units and records vacant ones', async () => {
  const { box, c, prop, unit, tenant } = await billedLease();
  const empty = (await c('create', { table: 'Units', data: { property_id: prop.id, unit_number: 'B' } })).data.row;
  const r = await c('billMeterReadings', { property_id: prop.id, category: 'Electricity', rate: 8.5,
  ...
  const line = (await box.readTable('InvoiceItems')).find(i => inv && i.invoice_id === inv.id);
  if (!line || Number(line.quantity) !== 142 || !/1000 → 1142/.test(line.description)) problems.push('line ' + JSON.stringify(line));
  const readings = await box.readTable('MeterReadings');
  ...
  if ((await c('remove', { table: 'Units', id: empty.id })).ok) problems.push('a unit with readings was deleted');
  return problems.length ? problems.join('; ') : null;
});

await probe('a reading lower than the last one is refused', async () => {
  const { box, c, prop, unit } = await billedLease();
  const r = await c('billMeterReadings', { property_id: prop.id, category: 'Water', rate: 1, reading_date: box.today(),
    readings: [{ unit_id: unit.id, previous_reading: 500, current_reading: 400 }] });
  return r.ok ? 'accepted a meter running backwards' : ((await box.readTable('MeterReadings')).length ? 'stored anyway' : null);
});
...
await probe('dates never drift, whatever zone the app runs in', async () => {
  ...
  if (snap.timezones.script !== 'America/Los_Angeles') return 'time zone not reported: ' + JSON.stringify(snap.timezones);
