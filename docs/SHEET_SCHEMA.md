...
A number is never issued twice: the highest one ever used is kept in the
script's properties (`SEQ_<tab>`), so deleting the newest row does not free its
number. Dates are stored as `yyyy-MM-dd` and read in the spreadsheet's own time
zone.

**New columns are added automatically.** When an updated `Code.gs` needs a column
your sheet does not have yet, the first signed-in request appends it (and any new
setting). Existing data is never touched.
...
## Tenants
`id · full_name · email · phone · alt_phone · id_type · id_number · occupation · emergency_name · emergency_phone · status · notes · created_at · updated_at · gstin`

`gstin` is optional, validated, and printed on tax invoices.
...
## Leases
`id · property_id · unit_id · tenant_id · start_date · end_date · rent_amount · deposit_amount · deposit_status · frequency · billing_day · late_fee · grace_days · escalation_pct · status · notes · created_at · updated_at · gst_rate · renewed_from`
...
- `billing_day` is **not used** by the app and is hidden from the form. Rent
  periods follow the lease start date (a lease starting on the 5th bills on the
  5th). The column remains so existing values are not lost.
- `late_fee` — added once, as a `Late Fee` line, when a rent invoice on the
  lease goes overdue. Never charged on the security-deposit invoice.

## Invoices
`id · lease_id · tenant_id · unit_id · property_id · type · period_start · period_end · issue_date · due_date · amount · tax · total · amount_paid · balance · status · notes · created_at · updated_at · cgst · sgst · igst · place_of_supply · last_reminded`

- `status` — Draft · Unpaid · Partial · Paid · Overdue · Void. A void invoice has a balance of 0.
- `tax` is the sum of the lines' GST, split into `cgst` + `sgst` or `igst`. An invoice whose lines carry no rate keeps a typed flat `tax`.
- `last_reminded` — the date a reminder last included this invoice.
- `type` — Rent · Deposit · Deposit Deduction · Electricity · Water · Gas · Mixed · …
...
## InvoiceItems
`id · invoice_id → Invoices.id · description · category · quantity · unit_amount · amount · notes · created_at · updated_at · tax_rate · tax_amount`
...
## MeterReadings
`id · property_id · unit_id · lease_id · tenant_id · category · reading_date · previous_reading · current_reading · consumption · rate · amount · invoice_id · notes · created_at · updated_at`

One row per meter read. `invoice_id` is blank for a unit that was vacant on the
reading date. The latest reading is where the next round starts.
...
- `password_hash` — `v2$` followed by a base64 SHA-256 of the salted password,
  stretched over 1,000 rounds. A value without the `v2$` prefix is the original
  single-round scheme and is upgraded the next time that account signs in.
...
| `reminder_overdue_days` | 1,7,14,30 | days overdue on which the daily job reminds |
...
| `upi_id` | — | shown on invoices and reminders, with a UPI payment link |
...
| Payments | Cannot exceed the outstanding balance, and a settled, void or draft invoice takes no more — whether recorded from the invoice or entered on the Payments page. Either way the invoice's balance and status follow at once, and the payment takes its tenant and property from the invoice |
...
| Deposits | Signing a lease with a deposit — or adding one to a lease that had none — raises a Deposit invoice, unless the lease already says Held. `deposit_status` is Pending until that invoice is fully paid, then Held — and back to Pending if the payment is reversed |
| Refunding a deposit | Refused while nothing is held. Marking it Refunded on the lease returns everything still held; *Settle deposit* applies it to arrears and deductions first. Changing the deposit amount re-prices its invoice, never below what has been paid |
| Text beginning with `=` | Stored as text, never as a formula, so a value typed into the app cannot read other tabs |
...
1. Add the column to the relevant tab's `SCHEMA` in `apps-script/Code.gs`. The
   first signed-in request after deploying adds it to the sheet, without
   touching data.
